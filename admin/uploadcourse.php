...
// Some Regex constants
    ...
    function validateas($value, $validatename, $lineno, $fieldname = '') {
        ...
        case 3: // Timestamp - validates and converts to Unix Time
            if ($value < 1)  // if (($value = strtotime($value)) < 1)  // CKM 2012-06-27 - is this strotitime a problem?
        ...
        foreach ($bulkcourses as $bulkcourse) {
            ...
            if (!fast_is_course($bulkcourse['shortname'])) {
                ...
                if ($coursetocategory==-1) {
                    ...
                    break;
                    case -4:
                        notify('Could not enroll teachers for course with ShortName '.$bulkcourse['shortname'],'notifyproblem');
                        $p++;
                    break;
                    case -5:
...
<tr><td>startdate</td><td>Unix timestamp</td></tr>
